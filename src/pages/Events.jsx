...
export default function Events() {
  ...
  return (
    <div className="min-h-[calc(100vh-8rem)] bg-slate-50">
      {/* Reuse the same flight search hero that is shown on the public home page */}
      <FlightSearchHero showAuthCtas={false} />

      <div className="container-app py-10">
        <div className="mb-8">
          <h1 className="page-heading text-slate-900">Search results (placeholder)</h1>
          <p className="mt-1 text-slate-600">
            This section will list available flights or bookings once the flight APIs are connected.
          </p>
        </div>
      <EventFilters filters={filters} onFilterChange={setFilters} categories={categories} />
        {loading ? (
          ...
          <div className="mt-10 text-center py-12 rounded-2xl bg-slate-50 border border-slate-100">
            <p className="text-slate-600">No data loaded yet.</p>
          </div>
        ) : (
          <>
